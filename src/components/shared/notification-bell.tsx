...
import { Bell, Clock, FileText, AlertOctagon, X } from "lucide-react";
...
export default function NotificationBell({ userId }: { userId: string }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <div className="fixed left-3 right-3 top-16 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 w-auto sm:w-80 bg-white border border-border rounded-xl shadow-lg z-50 overflow-hidden">
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[9px] text-foreground/35 mt-1 font-mono">{new Date(notif.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
              ))
