...
export function MfaStatusCard() {
  ...
  async function handleDisable() {
    if (!factorId) return;
    if (!window.confirm("Turn off two-factor authentication? You will be asked to set it up again on your next login.")) {
      return;
    }
  ...
  return (
    <Card>
      <CardContent className="pt-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {enabled
                ...
                : "Not enabled — required for this account type. You'll be asked to set it up on your next login."}
            </p>
          </div>
        </div>
        ...
        {enabled ? (
          <Button type="button" variant="outline" onClick={handleDisable} disabled={disabling}>
            {disabling ? "Turning off..." : "Turn Off Two-Factor Authentication"}
          </Button>
        ) : (
          <Button asChild>
            <Link href="/mfa-setup">Set Up Two-Factor Authentication</Link>
          </Button>
        )}
      </CardContent>
    </Card>
