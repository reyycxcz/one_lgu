...
import { createResident } from "@/actions/profile";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function CreateResidentSheet({ barangays }: CreateResidentSheetProps) {
      ...
      <SheetContent className="w-full sm:max-w-xl md:max-w-2xl p-6 flex flex-col justify-between overflow-y-auto">
        <div>
          <SheetHeader className="text-left space-y-1">
            <SheetTitle className="text-lg font-semibold text-foreground">Add Resident Account</SheetTitle>
            <SheetDescription className="text-xs text-muted-foreground">
              Register a resident account directly. Set a temporary password they can use to log in.
            </SheetDescription>
          </SheetHeader>

          <form id="create-resident-form" onSubmit={handleSubmit} className="mt-4 space-y-3.5">
            {error && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input name="email" type="email" placeholder="resident@example.com" required className="h-9 text-sm" />
              </div>
